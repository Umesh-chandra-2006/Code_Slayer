...
import { FaTachometerAlt, FaFlask, FaCode, FaPlus, FaSignOutAlt, FaUser, FaListAlt } from 'react-icons/fa';
...
const Sidebar = ({ isOpen, toggleSidebar }) => {
  ...
  const navItems = [
    ...
    { name: 'Create Problem', icon: FaPlus, path: '/problems/new' },
  ];
  ...
  return (
    <div
      className={`fixed top-0 left-0 h-full bg-gray-800 text-gray-300 flex flex-col
        ...
        ${isOpen ? 'w-48' : 'w-16'} /* Increased open width for text, still 64px for icons */
        lg:w-48 /* Keep this width on large screens */
        overflow-hidden /* Crucial for hiding overflowing content */
        ${isOpen ? 'p-4' : 'p-2'} /* Adjust padding based on state for visual comfort */
      `}
    >
      {/* Logo */}
      <div className="flex items-center justify-start h-16 bg-gray-900 text-blue-400 font-bold text-xl cursor-pointer mb-6">
        <Link to="/dashboard" className="flex items-center w-full h-full">
            <span className="text-2xl whitespace-nowrap ml-2">{'< >'}</span> {/* Adjust margin-left */}
            <span className={`transition-opacity duration-300 ${isOpen ? 'opacity-100 ml-2' : 'opacity-0'}`}>Judge</span>
        </Link>
      </div>
        ...
        <ul>
          {navItems.map((item) => (
            <li key={item.name} className="mb-2">
              <Link
                to={item.path}
                className={`flex items-center p-3 rounded-lg mx-auto ${isOpen ? 'justify-start' : 'justify-center'}
                  ${location.pathname === item.path ? 'bg-blue-600 text-white' : 'hover:bg-gray-700'}
                  transition-colors duration-200 ease-in-out w-full
                `}
              >
                <item.icon className={`h-6 w-6 ${isOpen ? 'mr-3' : 'mr-0'}`} /> {/* Adjust margin on icon */}
