...
export default function ViewProblem() {
  ...
  const fetchProblem = useCallback(async () => {
    ...
    try {
      ...
      const res = await axios.get(`${API_BASE_URL}/api/problems/${slug}`, {
        headers,
      });
      setProblem(res.data);
      ...
      const errorMessage =
        ...
        err.message ||
        "Failed to load problem details. It might not exist or you don't have permission.";
  ...
  useEffect(() => {
    if (problem && problem.title) {
      document.title = `${problem.title} | CodeSlayer`;
      ...
      metaDesc.content = `Solve the problem '${problem.title}' on CodeSlayer. Read the description, submit solutions, and view discussions.`;
    }
  }, [problem]);
  ...
  return (
    <>
      <title>[Problem Name] | CodeSlayer</title>
      <meta
        name="description"
        content="Solve the problem '[Problem Name]' on CodeSlayer. Read the description, submit solutions, and view discussions."
      />
      <div className="flex flex-col lg:flex-row min-h-screen bg-gray-900 text-gray-100">
        ...
        <div className="flex-1 overflow-y-auto p-6 lg:border-r border-gray-700 custom-scrollbar">
            ...
            <div className="mt-4 flex flex-wrap gap-2 mb-6">
              {problem.tags.map((tag, index) => (
                <Badge
                  key={index}
                  variant="info"
                  className="px-3 py-1 text-sm rounded-full bg-blue-700"
                >
                  {tag}
                </Badge>
              ))}
          ...
          <div className="p-4 border-b border-t border-gray-700 flex justify-between items-center mb-6 rounded-t-lg">
            <label
              htmlFor="problem-language-select"
                  ...
                  problem.sampleTestCases.length > 0 && (
                    <div className="my-6">
                      ...
                      {problem.sampleTestCases.map((sample, index) => (
                        <div
                          key={index}
                          className="p-4 border border-gray-600 rounded-lg bg-gray-800 mb-4 last:mb-0"
                        >
            ...
            {activeTab === "submissions" && (
              <div className="my-6">
                ...
                <div className="text-gray-400 leading-relaxed bg-gray-800 p-4 rounded-lg border border-gray-700">
                  <p className="mb-2">
                    This section will display your past submissions for this
                    problem.
                  </p>
