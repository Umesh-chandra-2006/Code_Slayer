...
export default function CodeEditor({
  ...
  const [initialProblemBoilerplateCode, setInitialProblemBoilerplateCode] =
    useState(initialCode);

  const [internalLanguage, setInternalLanguage] = useState(initialLanguage);
  const currentLanguage = isControlledByParent
    ? parentLanguage
    : internalLanguage;
  const currentSetLanguage = isControlledByParent
    ? setParentLanguage
    : setInternalLanguage;
  ...
  const handleRunOrTestCode = useCallback(
    async (e) => {
      e.preventDefault();
      clearResults();
      setLoadingRun(true);
      ...
      try {
        ...
        if (isProblemTestRun) {
          response = await axios.post(
            `${API_BASE_URL}/api/compiler/test`,
            {
        ...
        setRuntimeError(
          errorData.runtimeError ||
            errorData.error ||
            "Server error during code execution or testing."
        );
  ...
  const handleSubmitSolution = useCallback(async () => {
    ...
    if (!problemId || !userId) {
      setSubmissionMessage(
        "Error: Problem ID or User ID is missing for submission."
      );
  ...
  const handleAiReview = useCallback(async () => {
    ...
    try {
      const payload = {
        ...
        failedTestInput:
          submissionVerdict === "Wrong Answer" && testResults.length > 0
  ...
  return (
    <>
      <title>Code Editor | CodeSlayer</title>
      <meta
        name="description"
        content="Write, edit, and test your code solutions in the interactive CodeSlayer code editor."
      />
      <div className="flex flex-col h-full bg-gray-800 text-gray-100 rounded-lg shadow-lg">
        ...
        <div className="flex-1 p-4  flex flex-col">
          <label
            htmlFor="code-editor"
            className="block bg-gray-900 text-gray-300 text-sm font-semibold mb-2"
          >
            Code:
          </label>
            ...
            {currentTab === "output" && (
              ...
              >
                ...
                {!isRunVerdict &&
                  ...
                  submissionVerdict !== "Pending" && (
                    <p
                      className={`p-3 rounded-lg mb-4 text-sm font-bold ${statusVariant(
                        submissionVerdict
                      )}`}
                    >
                ...
                {(overallRunTime !== null || overallMemory !== null) && (
                  <div className="p-3 rounded-lg mb-4 text-sm bg-gray-700/50 text-gray-300 border border-gray-600 flex justify-between">
                    {overallRunTime !== null && (
                      <span>Overall Time: {overallRunTime.toFixed(2)} ms</span>
                    )}
                    {overallMemory !== null && (
                      <span>Overall Memory: {overallMemory} KB</span>
                    )}
                  </div>
                )}
                {output || compilationError || runtimeError ? (
                  <div className="bg-gray-900 p-4 rounded-lg border border-gray-700 min-h-[100px] overflow-auto">
                    ...
                    {compilationError && (
                      <>
                        <h4 className="text-red-400 font-semibold mb-2 mt-4">
                          Compilation Error:
                        </h4>
                    ...
                    {runtimeError && (
                      <>
                        <h4 className="text-yellow-400 font-semibold mb-2 mt-4">
                          Runtime Error:
                        </h4>
                        <pre className="text-sm text-yellow-300 whitespace-pre-wrap">
                          {runtimeError}
                        </pre>
                      </>
                    )}
                  ...
                  <p className="text-gray-400 text-center py-4">
                    Run your code to see output or errors here.
                  </p>
                )}
              </motion.div>
            )}
                  ...
                  !loadingSubmit && (
                    <div className="space-y-4 mb-4">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                            <div>
                              <p className="text-gray-300">
                                ...
                                <pre className="inline-block bg-gray-700/50 p-1 rounded whitespace-pre-wrap">
                                  {tc.input}
                                </pre>
                              </p>
                            </div>
                      ...
                      <p className="text-gray-400 text-center py-2">
                        Run your code to see results for these tests.
                      </p>
                    </div>
                  )}
                ...
                {testResults.length > 0 ? (
                  <div className="space-y-4">
                    {testResults.map((result, index) => (
                      <div
                        key={index}
                        className={`p-4 rounded-lg border ${statusVariant(
                      ...
                      >
                        ...
                        {result.details && (
                          <p className="text-red-300 mt-2">
                            <strong className="text-red-200">Details:</strong>{" "}
                            {result.details}
                          </p>
                        )}
                      </div>
                  ...
                  (!problemId || publicTestCases.length === 0) && (
                    <p className="text-gray-400 text-center py-4">
                      Test all cases to see results here.
                    </p>
                  )
                )}
                {compilationError && (
            ...
            {currentTab === "aiReview" && (
              ...
              >
                ...
                {loadingAiReview ? (
                  ...
                  <p className="text-gray-400 text-center py-4">
                    Click "AI Review" after a submission to get insights.
                  </p>
                )}
              </motion.div>
            )}
